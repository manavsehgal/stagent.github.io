...
- **Dev Mode Gate** — When `STAGENT_DEV_MODE=true` is set, the Instance section displays a "Dev mode" banner and the upgrade machinery is intentionally bypassed, protecting contributor repos from having a pre-push hook installed by the bootstrap flow.
- **Hybrid Licensing** — Local-only features (task execution, workflows, schedules, tables) remain unlimited regardless of license state. Cloud features (sync, marketplace, team sharing) are metered via a `(email, machineFingerprint, instanceId)` tuple so the same user on the same machine counts as one seat across multiple instance branches.
